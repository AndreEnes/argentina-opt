...
def fitness_function(x, target_output, features_space):
    ...
    x_concat_df = pd.DataFrame(x_concat)
    x_concat_fixed = np.transpose(x_concat_df.values)
    curr_value = model.predict(x_concat_fixed)
...
def __discrete_params_optimization__(features_space, x0, target_output, model, cb=dual_annealing_callback):
    x0_columns = []
    ...
    if 'target_output' not in st.session_state:
        ...
        if st.button('Optimize parameters'):
            ...
            if 'df_vars_cleaned' not in st.session_state:
                ...
                curr_pred = model.predict(st.session_state['opt_values_df'].iloc[0:1].values)
                ...
                params_fixed = np.transpose(pd.DataFrame(params))
                pred_output_str = '**Predicted output**: ' + str('{:10.4f}'.format(float(model.predict(params_fixed))))
