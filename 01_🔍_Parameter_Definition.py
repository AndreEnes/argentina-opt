...
|{str('{:10.4f}'.format(test_r2))} | {str('{:10.4f}'.format(test_mae))} | {str('{:10.4f}'.format(test_mse))} |"""
                    ...
                    __write_json__(st.session_state['model_error'], 'model_error', json_file)

                    df_json_str = str(st.session_state['root_dir']) + '\\df.json' 
                    df_no_output_json_str = str(st.session_state['root_dir']) + '\\df_no_output.json'
                    df.to_json(df_json_str, indent=4)
                    df_no_output.to_json(df_no_output_json_str, indent=4)
