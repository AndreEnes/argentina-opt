...
if 'root_dir' not in st.session_state:
    ...
    if st.button('Select'):
        ...
        if not(os.path.exists(params_check_str)):  
            ...
            st.session_state['model_error'] = data['model_error'][0]

            model_file = str(st.session_state['root_dir']) + '\\model.sav'
            st.session_state['model'] = pickle.load(open(model_file, 'rb'))
            
            df_str = str(st.session_state['root_dir']) + '\\df.json'
            df_no_output_str = str(st.session_state['root_dir']) + '\\df_no_output.json'
            st.session_state['df'] = pd.read_json(df_str)
            st.session_state['df_no_output'] = pd.read_json(df_no_output_str)
else:
